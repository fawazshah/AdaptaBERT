...
from pytorch_pretrained_bert.modeling import BertPreTrainedModel, BertModel, BertOnlyMLMHead, WEIGHTS_NAME
...
class BERTDataset(Dataset):
    def __init__(self, data_dir, tokenizer, seq_len):
        ...
        trg_domain_examples = processor.get_sep_twitter_train_examples(data_dir)
        # use test examples in unsupervised domain tuning
        trg_domain_examples.extend(processor.get_sep_twitter_test_examples(data_dir))

        self.examples = trg_domain_examples

        # Include equal amount of src domain data for NSP, or all src data if not enough

        src_domain_examples = processor.get_conll_train_examples(data_dir)
        src_domain_examples.extend(processor.get_conll_dev_examples(data_dir))
